...
import fileIO
import interfacing
#import LocalInterfacing as interfacing
import serial
import struct
	...
	def run(self):
		...
		while not self.quit:
			...
			if self.shiftXVal != tempShiftXVal or self.shiftYVal != tempShiftYVal or self.stigXVal != tempStigXVal or self.stigYVal != tempStigYVal or self.condVal != tempCondVal or self.objVal != tempObjVal or self.filaVal != tempFilaVal or self.magVal != tempMagVal:
				...
				#dev.write(0x01, 'this is a message')
				#dev.read(0x82, numBytes)
				'''
				global pool
...
def makePixbuff( byteList , width, height ):
	...
	for i in byteList:
		array = array +  i + i + i
	pixbuff = gtk.gdk.pixbuf_new_from_data( array, gtk.gdk.COLORSPACE_RGB, False, 8, width, height, width)
	return pixbuff
