...
class commThread(threading.Thread):
	def __init__( self ):
		...
		try:
			self.serial = serial.Serial(port='COM5', baudrate = 115200, bytesize=8, parity='N', stopbits=1, timeout = 1)
	...
	def run(self):
		...
		global magVal
		while not self.quit:
			readLock.acquire(True) #we want this to block
			...
			if self.shiftXVal != tempShiftXVal or self.shiftYVal != tempShiftYVal or self.stigXVal != tempStigXVal or self.stigYVal != tempStigYVal or self.condVal != tempCondVal or self.objVal != tempObjVal or self.filaVal != tempFilaVal or self.magVal != tempMagVal:
				...
				global pool #this is a workaround: for robustness, the methods should be altered
				interfacing.sendPresets(pool)
...
class MainWindow:
	def updateVars(self):
		...
		global updateFlag
		if updateFlag:
			if readLock.acquire(False):	#not blocking, true if lock acquired
				global shiftXVal
				global shiftYVal
				global stigXVal
				global stigYVal
				...
				updateFlag = False
				readLock.release()
			else:				#the other thread holds the lock, and we readd this method to the idle thread
	...
	def adjusted(self, widget, inVal=None): 
		#xshift, yshift, xstig, ystig, cond, obj, fila, mag is the order
		#this method is called whenever a slider is adjusted
		global updateFlag
		if inVal == 1: 
	...
	def makeScreenshot(self, widget, data=None):
		...
		if not self.serial.isOpen:
			...
			return
		self.serial.write( chr(0b11111000) )
		
		widthBytes = [ 0, 0 ]
		heightBytes = [ 0, 0 ]
		inVal = self.serial.read()
		if inVal != chr(0b00001111):
			return
		counter = 0
		while counter < 4:
			inVal = self.serial.read()
			if counter < 2:
				widthBytes[counter] = inVal
			else:
				heightBytes[counter-2] = inVal
			counter = counter + 1
		
		#given a little endian pair of bytes, this gives us their values
		width = 256 * struct.unpack('B', widthBytes[0])[0] + struct.unpack('B', widthBytes[1])[0] 
		height = 256 * struct.unpack('B', heightBytes[0])[0] + struct.unpack('B', heightBytes[1])[0]
		
		imageData = ''
		readVal = ' '
		counter = 0	
		date = time.strftime("%Y-%m-%d_%H_%M_%S", time.gmtime())
		filename = "SEM image "+date+".jpg"
		print "width is " + str(width)
		print "height is " + str(height)
		imageData = []
		for i in range(height):
			rawData = self.serial.read(width)
			imageData.append(rawData)
		
		print "done receiving"
		
		pixbuf = makePixbuff(imageData, width, height)
	...
	def __init__(self):
		...
		#make serial connection
		try:
			self.serial = serial.Serial(port='COM5', baudrate = 115200, bytesize=8, parity='N', stopbits=1, timeout = 2)
			self.serial.close()
		except serial.serialutil.SerialException:
			self.serial = None
...
def makePixbuff( byteList , width, height ):
	imageList = []
	for row in byteList:
		row = [ [x, x, x] for x in row ]
